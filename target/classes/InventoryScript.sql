...
CREATE TABLE grv_master (
    ...
    -- need to add foreign key location_id
);
...
CREATE TABLE grv_material_detail (
    ...
    -- // need to add uom id, material_code foreign key
);
...
CREATE TABLE grn_material_detail(
    ...
    gi_sub_process_id INT NOT NULL,
    gi_process_id VARCHAR(50) NOT NULL,
    quantity BIGDECIMAL(10, 2) NOT NULL,
    ...
    FOREIGN KEY (gi_sub_process_id) REFERENCES goods_inspection_master(sub_process_id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE ohq_master (
    ...
    FOREIGN KEY (asset_id) REFERENCES asset_master(asset_id) ON UPDATE CASCADE
    --  // need to add locator id foreign key
)
